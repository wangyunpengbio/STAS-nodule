...
def average_batch_features_in(features_hook, times):
    assert len(features_hook) == times
    features_hook_processed = features_hook[0][0]
    for item in features_hook[1:]:
        features_hook_processed += item[0]
    features_hook_processed = features_hook_processed / times
    return features_hook_processed


def average_batch_features_out(features_hook, times):
    assert len(features_hook) == times
    features_hook_processed = features_hook[0]
    for item in features_hook[1:]:
        features_hook_processed += item
    features_hook_processed = features_hook_processed / times
    return features_hook_processed

def get_intermediate_epoch(model, device, data_loader, times=20):
    img_name_list = []
    # 定义hook对应的list，把方法注册以后，网络向前传播会直接将中间层的值 放到list里面
    features_in_hook, features_out_hook = [], []
    features_all_in_list, features_all_out_list = [], []
    features_all_in, features_all_out = None, None
    ...
    tta_transform = Compose(
        [
            ...
            RandRotate90d(keys=["img"], prob=0.5, spatial_axes=(1, 2)),
            RandRotate90d(keys=["img"], prob=0.5, spatial_axes=(1, 3)),
            RandRotate90d(keys=["img"], prob=0.5, spatial_axes=(2, 3)),
        ]
    )
    ...
    with torch.no_grad():
        for val_data in data_loader:
            tta_data = val_data[0]  # 就一个样本
            outputs_list, outputs_argmax_list = [], []
            for time_idx in range(times):
                tta_data_transformed = tta_transform(tta_data)
                tta_images, tta_labels = tta_data_transformed["img"].to(device), tta_data_transformed["label"].to(device)
                outputs = model(tta_images)
                outputs_list.append(outputs)
                outputs_argmax = outputs.argmax(dim=1)
                outputs_argmax_list.append(outputs_argmax)

            features_batch_in = average_batch_features_in(features_in_hook, times)
            features_batch_out = average_batch_features_out(features_out_hook, times)
            features_in_hook, features_out_hook = [], []
            features_batch_in = features_batch_in.cpu().detach().numpy()
            features_batch_out = features_batch_out.cpu().detach().numpy()
            # features_all_in_list.append(features_batch_in)
            # features_all_out_list.append(features_batch_out)

            if features_all_in is None:
                features_all_in = features_batch_in
            else:
                features_all_in = np.concatenate((features_all_in, features_batch_in), axis=0)
            if features_all_out is None:
                features_all_out = features_batch_out
            else:
                features_all_out = np.concatenate((features_all_out, features_batch_out), axis=0)
            # outputs_tensor = torch.stack(outputs_list)
            # outputs_mean_tensor = torch.mean(outputs_tensor, dim=0)
            # y_pred = torch.cat([y_pred, outputs_mean_tensor], dim=0)
            # y = torch.cat([y, tta_labels], dim=0)
            img_name_list.extend(tta_data['img_meta_dict']['filename_or_obj'])


    # features_all_in = np.array(features_all_in_list)
    # features_all_out = np.array(features_all_out_list)
    # features_in = features_all_in.reshape((-1, features_batch_in.shape[1]))
    # features_out = features_all_out.reshape((-1, features_batch_out.shape[1]))
    features_in = features_all_in
    features_out = features_all_out
...
def main():
    ...
    intermediate_save_name_train = join(save_dir, 'tta_{}_train.csv'.format(opt.pth_name))
    ...
    intermediate_save_name_val = join(save_dir, 'tta_{}_val.csv'.format(opt.pth_name))
    ...
    intermediate_save_name_test = join(save_dir, 'tta_{}_test.csv'.format(opt.pth_name))
