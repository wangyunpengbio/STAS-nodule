...
def calculate_roc(dataset, train_col, criterion, max_depth_tree, whether_using_dl_feature):
    ...
    tree_cls = tree.DecisionTreeClassifier(criterion=criterion, max_depth=max_depth_tree, random_state=2022)
    tree_cls.fit(X_train, y_train)

    train_auc_rf = calculate_auc(tree_cls, X_train, y_train)
    val_auc_rf = calculate_auc(tree_cls, X_val, y_val)
    test_auc_rf = calculate_auc(tree_cls, X_test, y_test)
    roc_dict['tree'].extend([train_auc_rf, val_auc_rf, test_auc_rf])
...
def parse_args():
    parse = argparse.ArgumentParser(description='Tree Model + Deep Learning model')
...
if __name__ == '__main__':
    ...
    max_depth_tree_list = [1, 2, 3, 4, 5, 6]

    param_grid_list = list(itertools.product(criterion_list, max_depth_tree_list))
    for (criterion, max_depth_tree) in tqdm(param_grid_list):
        for tag in [tag_id, tag_id + 1]:
            ...
            for i in range(5): # 指定fold数目
                for target_epoch in target_epoch_list:
                    ...
                    roc_dict = calculate_roc(dataset_merged, train_col, criterion, max_depth_tree, whether_using_dl_feature)
                    ...
                    for item in range(len(value_list)):
                        ...
                        result_dict['criterion'].append(criterion)
                        result_dict['max_depth_tree'].append(max_depth_tree)
    result_df = pd.DataFrame.from_dict(result_dict)
    if whether_using_dl_feature:
        tag_id = "{}_dl".format(tag_id)
    else:
        tag_id = "{}_nodl".format(tag_id)
    result_df.to_csv(join(out_dir, "all_tree_0{}.xlsx".format(tag_id)), sep='\t')
    summarized_df = result_df.groupby(["hospital", "epoch", "split", "criterion", "max_depth_tree"])[['tree']].mean()
    # summarized_df = summarized_df.reset_index(level=[0, 1, 2])
    summarized_df.to_csv(join(out_dir, "mean_tree_0{}.xlsx".format(tag_id)), sep='\t')
    print(summarized_df)
