...
# ridge岭回归是做数值回归的，目标是 负无穷到正无穷，所以要算一下点到超平面的距离作为置信度，再加个softmax
...
def calculate_roc(dataset, train_col, alpha, penalty, whether_using_dl_feature):
    ...
    ridge = RidgeClassifierwithProba(alpha=alpha, random_state=2022)
    ridge.fit(X_train, y_train)

    # ConvergenceWarning: The max_iter was reached which means the coef_ did not converge ConvergenceWarning,
    logistic = linear_model.LogisticRegression(penalty=penalty, solver='saga', max_iter=5000, random_state=2022)
    logistic.fit(X_train, y_train)

    train_auc_rf = calculate_auc(ridge, X_train, y_train)
    val_auc_rf = calculate_auc(ridge, X_val, y_val)
    test_auc_rf = calculate_auc(ridge, X_test, y_test)
    roc_dict['ridge'].extend([train_auc_rf, val_auc_rf, test_auc_rf])

    train_auc_rf = calculate_auc(logistic, X_train, y_train)
    val_auc_rf = calculate_auc(logistic, X_val, y_val)
    test_auc_rf = calculate_auc(logistic, X_test, y_test)
    roc_dict['logistic'].extend([train_auc_rf, val_auc_rf, test_auc_rf])
...
def parse_args():
    parse = argparse.ArgumentParser(description='Linear Model + Deep Learning model')
...
if __name__ == '__main__':
    ...
    alpha_list = list(np.logspace(-6, 6, 13))
    penalty_list = ['l1', 'l2', 'none']

    param_grid_list = list(itertools.product(alpha_list, penalty_list))
    for (alpha, penalty) in tqdm(param_grid_list):
        for tag in [tag_id, tag_id + 1]:
            ...
            for i in range(5): # 指定fold数目
                for target_epoch in target_epoch_list:
                    ...
                    roc_dict = calculate_roc(dataset_merged, train_col, alpha, penalty, whether_using_dl_feature)
                    ...
                    for item in range(len(value_list)):
                        ...
                        result_dict['penalty'].append(penalty)
    result_df = pd.DataFrame.from_dict(result_dict)
    if whether_using_dl_feature:
        tag_id = "{}_dl".format(tag_id)
    else:
        tag_id = "{}_nodl".format(tag_id)
    result_df.to_csv(join(out_dir, "all_linearmodel_0{}.xlsx".format(tag_id)), sep='\t')
    summarized_df = result_df.groupby(["hospital", "epoch", "split", "alpha", "penalty"])[['ridge', 'logistic']].mean()
    # summarized_df = summarized_df.reset_index(level=[0, 1, 2])
    summarized_df.to_csv(join(out_dir, "mean_linearmodel_0{}.xlsx".format(tag_id)), sep='\t')
    print(summarized_df)
